...
function CategoryCard({ title, subtitle, headerIcon: HeaderIcon, children, delay, viewProjects, className = "" }) {
  return (
    <motion.div
      ...
      className={`rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0d0d1a] p-6 flex flex-col ${className}`}
    >
      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 rounded-xl bg-violet-100 dark:bg-violet-600/20 flex items-center justify-center flex-shrink-0">
          <HeaderIcon size={20} className="text-violet-600 dark:text-violet-400" />
        </div>
        <div>
...
function SkillTile({ item }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0f0f20] hover:border-violet-400 dark:hover:border-violet-400 hover:-translate-y-1 transition-all cursor-default">
      <div className={`w-11 h-11 flex items-center justify-center rounded-lg ${item.badgeBg || ""}`}>
        <Icon size={22} className={item.iconColor} />
      </div>
      <div className="text-center">
        <p className="text-xs font-semibold text-gray-800 dark:text-white leading-tight">{item.name}</p>
        {item.tagline && (
          <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-tight mt-1">{item.tagline}</p>
        )}
      </div>
    </div>
  );
...
function OtherSkillPill({ skill }) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0f0f20] text-sm font-medium text-gray-700 dark:text-gray-300 hover:border-violet-400 dark:hover:border-violet-400 transition-colors cursor-default">
...
function Skills() {
  return (
    <section id="skills" className="relative bg-white dark:bg-[#05050a] px-8 md:px-16 py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          className="mt-6 rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0d0d1a] px-8 py-5 flex items-center"
        >
          <span className="text-4xl font-serif text-violet-500 dark:text-violet-400 mr-4 leading-none select-none">"</span>
